"""
Original code: https://github.com/mlfoundations/open_clip/blob/v2.24.0/src/training/main.py
"""
import glob
import logging
import os
...
BEST_UNC_CHECKPOINT_NAME = "epoch_best_unc.pt"
...
def main(args):
    ...
    model, preprocess_train, preprocess_val = create_model_and_transforms(
        ...
        output_dict=True,
        reinit_unc=args.reinit_unc,
        force_resize_pos_emb=args.force_resize_pos_emb,
        n_cls=args.force_resize_n_cls,
        **model_kwargs,
    )
    if args.distill:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.save_logs:
            ...
            if args.save_best_unc and metrics:
                unc = metrics["unc/imagenet-zeroshot-val-unc-vs-acc-top1"]
                if prev_best_unc > unc:
                    prev_best_unc = unc
                    tmp_save_path = os.path.join(args.checkpoint_path, "tmp.pt")
                    torch.save(checkpoint_dict, tmp_save_path)
                    best_unc_save_path = os.path.join(args.checkpoint_path, BEST_UNC_CHECKPOINT_NAME)
                    os.replace(tmp_save_path, best_unc_save_path)
