        ...
        if self.world_size > 1:
            ...
            if self.bidir:
                ...
                for i in range(num_bidir):
                    ...
                    text_sigmas_recv = neighbour_exchange_bidir_with_grad(
                        left_rank,
                        right_rank,
                        text_sigmas_to_left,
                        text_sigmas_to_right,
                    )

                    for mu, sigma in zip(text_features_recv, text_sigmas_recv):
                        _loss, _loss_dict = self._loss(
                            image_features,
                            None,
                            logit_scale,
                            logit_bias,
                        ...
                        loss += _loss
                        loss_dict = {k: v + _loss_dict[k] for k, v in loss_dict.items()}
                    text_features_to_left, text_features_to_right = text_features_recv
                    text_sigmas_to_left, text_sigmas_to_right = text_sigmas_recv
