...
Original code: https://github.com/mlfoundations/open_clip/blob/v2.24.0/src/open_clip/model.py
"""
import copy
import logging
import math
...
def _build_vision_tower(
    ...
    if vision_cfg.timm_model_name:
        raise ValueError("TIMM is not supported")
    elif isinstance(vision_cfg.layers, (tuple, list)):
        vision_heads = vision_cfg.width * 32 // vision_cfg.head_width
        raise ValueError("Modified ResNet is not supported")
    else:
...
def _build_text_tower(
    ...
    if text_cfg.hf_model_name:
        raise ValueError("HFTextEncoder is not supported")
    else:
        act_layer = QuickGELU if quick_gelu else nn.GELU
        norm_layer = LayerNormFp32 if cast_dtype in (torch.float16, torch.bfloat16) else LayerNorm
