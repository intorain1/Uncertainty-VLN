"""
Original code: https://github.com/mlfoundations/open_clip/blob/v2.24.0/src/open_clip/transform.py
"""
import numbers
import random
import warnings
    ...
    if is_train:
        ...
        if use_timm:
            ...
            if aug_cfg.color_jitter_prob:
                ...
                aug_cfg_dict.pop('color_jitter_prob')
                aug_cfg_dict.pop('color_jitter')
            if aug_cfg.gray_scale_prob:
                train_transform.extend([
                    gray_scale(aug_cfg.gray_scale_prob)
                ])
                aug_cfg_dict.pop('gray_scale_prob')
            train_transform.extend([
                ToTensor(),
                normalize,
            ])
